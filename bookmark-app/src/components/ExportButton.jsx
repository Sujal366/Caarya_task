...
const ExportButton = ({className}) => {
  ...
  return (
    <div className={`relative text-sm md:text-base ${className}`}>
      <div
        onClick={() => setDropdownOpen(!dropdownOpen)}
        className="cursor-pointer flex items-center gap-2 p-2 bg-white text-gray-700 rounded sm:rounded-md hover:bg-blue-500 hover:text-white text-sm md:text-base border border-gray-300"
      >
        <p>Export Data</p>
      </div>
      {dropdownOpen && (
